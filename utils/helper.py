...
class BaseHelper(metaclass=SingletonMeta):
    ...
    def _load_json(self, file_path: Path) -> Dict[str, Any]:
        try:
            ...
            raise RuntimeError(f"Failed to parse JSON file: {file_path}") from e
    ...
    def _read_file(self, file_path: Path, cache_key: str) -> Dict[str, Any]:
        if cache_key not in self._cache:
            ...
            if file_path.suffix == ".json":
                self._cache[cache_key] = self._load_json(file_path)
            else:
...
class ConfigHelper(BaseHelper):
    ...
    def _apply_env_vars(data: Dict, prefix: str = _ENV_VAR_PREFIX) -> None:
        ...
        Mapping: TEST_AUTH__USERNAME -> data["auth"]["username"]
        - prefix is stripped
    ...
    def get(self, category: str, default: Any = _UNSET) -> Any:
